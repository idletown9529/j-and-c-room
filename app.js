...
function main(){
  ...
  $('accept').style.display=pending?'inline-flex':'none';$('redraw').style.display=pending?'inline-flex':'none';
  if(!pending){$('label').textContent='还没抽签';$('resTitle').textContent=active==='diy'?'先在下面写入你的自定义签，再抽。':'把手伸进签筒里，阿迟替你摸一张。';$('resNote').textContent='抽到后可以重抽，也可以收下。只有收下才会进入记录。'}
...
function acceptOne(){if(!pending)return;let j=jars[active];records.push({id:uid(),ts:Date.now(),date:date(),time:time(),key:active,jarName:j.name,title:pending.title,note:pending.note||'',mode:modeLabel(active)});pending=null;save();render();say('已收下这签')}
...
function bind(){
  $('draw').onclick=drawOne;$('redraw').onclick=drawOne;$('accept').onclick=acceptOne;$('clearJar').onclick=clearJarToday;$('clearToday').onclick=clearTodayAll;
